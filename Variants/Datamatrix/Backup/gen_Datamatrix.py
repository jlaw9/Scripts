...
def main(project_dir, hotspot, out_file):
	...
	writeVarInfo(all_vars_info, list_all_vars, 'var_info.csv')
...
# all_var_info is a global dictionary containing all of the variants and their counts and GT frequencies.
...
def add_var_info(chr_pos, GT, freq):
	...
	if GT in all_var_info[chr_pos]:
		all_vars_info[chr_pos][GT] += 1
	else:
		all_vars_info[chr_pos][GT] = 1
	if 'freq' in all_var_info[chr_pos]:
		all_vars_info[chr_pos]['freq'].append(freq)
	else:
		all_vars_info[chr_pos]['freq'] = [freq]

def writeVarInfo(all_vars_info, list_all_vars, out_file):
	with open(out_file, 'w') as out:
		for var_list in list_all_vars:
			chr_pos = var_list[-1]
			out.write(str(var_list) + str(all_vars_info[chr_pos]))
