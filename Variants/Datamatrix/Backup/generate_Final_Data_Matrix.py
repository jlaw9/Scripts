		...
		for line in anno_vcf_file:
			if line[0] != '#' and line[0:3] == "chr" :
				...
				if not options.multi_var_only or len(line[4].split(',')) > 1:
					...
					total_depth, num_alleles, GT_Info = getVarInfo(line, .2, .8)
					...
					if total_depth < DEPTH_CUTOFF:
						...
						# add two alleles listed at this position to the dictionary to be output.
...
def getVarInfo(line, WT_cutoff, HOM_cutoff):
		...
		if total_depth != 0:
			...
			if len(freqs) > 2:
				...
				if freqs[0][0] == ref_allele[0]:
					...
					elif freqs[0][1] < WT_cutoff:
						# this should be HOM mutant
						return total_depth, 2, [freqs[0][0], freqs[0][0], str((freqs[0][1]))[0:5]]
