...
def write3x3Tables(QC_comparisons):
	...
	if not options.sheet_per_sample:
		global MRsheet
		MRsheet = workbook.add_worksheet("3x3 tables")
	row = 2
	col = 2
	max_runs = 0
	# first sort the QC comparisons so they'll be pretty
	for sample in sorted(QC_comparisons):
		num_runs = 0
		for runs_compared, table_values in QC_comparisons[sample].iteritems():
			num_runs += 1
			if re.search('T-', runs_compared):
				tumor = True
			elif re.search('N-', runs_compared):
				normal = True
			elif re.search('Run', runs_compared):
				germline = True
	...
	if tumor and normal:
		for sample in sorted(QC_comparisons):
			for runs_compared in QC_comparisons[sample]:
				if re.search("N-", runs_compared.split('vs')[0]) and re.search("N-", runs_compared.split('vs')[1]):
					normals.append(runs_compared)
				if re.search("T-", runs_compared.split('vs')[0]) and re.search("T-", runs_compared.split('vs')[1]):
					tumors.append(runs_compared)
				else:
					tumor_normals.append(runs_compared)
			if options.sheet_per_sample:
				sample_sheet = workbook.add_worksheet(sample + " 3x3")
				row, col = triangulateRuns(normals, QC_comparisons[sample], row, col, sample_sheet)
				row, col = triangulateRuns(tumors, QC_comparisons[sample], row, col, sample_sheet)
				row, col = writeRuns(tumor_normals, QC_comparisons[sample], row, col, sample_sheet)
			else:
				row, col = triangulateRuns(normals, QC_comparisons[sample], row, col, MRsheet)
				row, col = triangulateRuns(tumors, QC_comparisons[sample], row, col, MRsheet)
				row, col = writeRuns(tumor_normals, QC_comparisons[sample], row, col, MRsheet)
	else:
		for sample in sorted(QC_comparisons):
			if options.sheet_per_sample:
				sample_sheet = workbook.add_worksheet(sample + " 3x3")
				triangulateRuns(QC_comparisons[sample], row, col, sample_sheet)
			elif max_runs > 4:
				row, col = triangulateRuns(QC_comparisons[sample], row, col, MRsheet)
			else:
				row, col = writeRuns(QC_comparisons[sample], row, col, MRsheet)


# @param runs_compared the title of the table
# @param table_values the dictionary of info for the tables
def writeTable(runs_compared, table_values):
	#first make the header for each table.
	header = "%s   -  vs  -   %s"%(runs_compared.split('vs')[0], runs_compared.split('vs')[1]) # i.e. sample1  Run1  -  vs  - Run2
