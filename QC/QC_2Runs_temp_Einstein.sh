...
#else
# TEMP Need to fix the total_eligible_bases in Einstein and SEGA.
	...
	intersected_bed="${TEMP_DIR}/intersect_${bed_name}"
	...
	if [ "`find ${OUTPUT_DIR}/Run1${CHR}_depths -maxdepth 0 -type f 2>/dev/null`" -a "`find ${OUTPUT_DIR}/Run2${CHR}_depths -maxdepth 0 -type f 2>/dev/null`" ]; then
		# TEMP script I wrote to correct that samtools does not write positions that have 0 depth
		total_eligible_bases=`python ${QC_SCRIPTS}/QC_Match_Depths.py ${OUTPUT_DIR}/Run1${CHR}_depths ${OUTPUT_DIR}/Run2${CHR}_depths $DEPTH_CUTOFF1 $DEPTH_CUTOFF2`
	elif [ "`find ${OUTPUT_DIR}/Both_Runs_${CHR}depths -maxdepth 0 -type f 2>/dev/null`" ]; then
		...
		echo "${RUN1_DIR}/PTRIM.bam" > ${TEMP_DIR}/depths_files 
		echo "${RUN2_DIR}/PTRIM.bam" >> ${TEMP_DIR}/depths_files 
